...
function bk_Z(p, c, params, constraints, rng)
...
function bk_gradZ(p, c, params, constraints, rng)
    f = let p=p, c=c, constraints=constraints, rng=rng
            pt -> bk_Z(p, c, pt, constraints, rng)
    end
    ForwardDiff.gradient(f, params)
end

function bk_updateParams(p, c, params, constraints, epsilon, rng)
    ne = eta_eps(epsilon)
    gradients = bk_gradZ(p, c, params, constraints, rng)
    normalisation = bk_Z(p, c, params, constraints, rng) 
...
function bk_entropy(p, c, params, constraints, epsilon, rng)
    ne = eta_eps(epsilon)
    Q, R, V = bk_fromParamsToMatrices(params)
    s = log(2)
    s -= 0.5 * tr(R * Q) # R and Q have zero diagonal and are symmetric
    s -= (ne == 0 ? 0 : tr(V*V) / (2*ne))
    s += log(bk_Z(p, c, params, constraints, rng))
    s
end

function bk_solve(p::Int, c::Int, dyn_params, epsilon; 
    init = [],
    iters = 500,
    beta = 1.0,
    tol = 1e-3,
    ...
    function updater!(F, x)
        F[:] = bk_updateParams(p, c, x, constraints, epsilon, rng)
    end
    ...
    norm_entropy = !converged(res) ? NaN : bk_entropy(p, c, res.zero, constraints, epsilon, rng)/log(2)
...
function bk_sweep_kappa(
    p::Int, c::Int, dyn_params, epsilon; 
    init = [],
    iters = 500,
    tol = 1e-3,
    verbose = false,
